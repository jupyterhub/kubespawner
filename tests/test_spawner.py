    ...
    if ssl_ca:
        ...
        if ssl_client_cert:
            context.load_cert_chain(
                certfile=ssl_client_cert, keyfile=ssl_client_key
            )
    else:
        context = None
...
async def test_spawn_start(
    kube_ns, kube_client, config, hub, exec_python,
):
    spawner = KubeSpawner(
        hub=hub,
...
async def test_spawn_internal_ssl(
    ...
    hub_ssl_ca = os.path.join(
        hub_ssl_dir, ssl_app.internal_trust_bundles["hub-ca"]
    )
...
async def test_pod_ip_template(kube_ns, kube_client, config):
    config.KubeSpawner.pod_ip_template = (
        "jupyter-{username}--{servername}.foo.example.com"
    )
    ...
    spawner = KubeSpawner(
        hub=hub, user=user, config=config, orm_spawner=MockOrmSpawner(),
    )
