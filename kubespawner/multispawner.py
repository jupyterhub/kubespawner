...
class MultiNamespacedKubeSpawner(KubeSpawner):
    ...
    delete_grace_period = 3  # Much shorter than the default 30
    ...
    def __init__(self, *args, **kwargs):
        _mock = kwargs.get('_mock')  # Not pop: superclass needs it
        ...
        try:
            ...
            if _mock:
                self.log.error(
                    "Got exception '{}' but _mock is set".format(exc))
            else:
                raise

        selected_pod_reflector_classref = MultiNamespacePodReflector
        selected_event_reflector_classref = MultiNamespaceEventReflector
        ...
        self.__class__.pod_reflector = selected_pod_reflector_classref(
            parent=self, on_failure=on_pod_reflector_failure,
            namespace=self.namespace)
        self.log.debug("__init__: Created new pod reflector: {}".format(
            self.__class__.pod_reflector))
        # And event_reflector.
        self.__class__.event_reflector = selected_event_reflector_classref(
            parent=self, namespace=self.namespace)
        self.log.debug("__init__: Created new event reflector: {}".format(
            self.__class__.event_reflector))
        # Start the reflectors
        self._start_watching_pods(replace=True)
        if self.events_enabled:
            self._start_watching_events(replace=True)
        self.log.debug("MultiNamespacedKubeSpawner initialized.")
    ...
    def poll(self):
        ...
        p_reflec = self.pod_reflector
        p_key = self._get_pod_key()
        data = p_reflec.pods.get(p_key, None)
        if data is not None:
    ...
    def _start(self):
        ...
        # Go forth and poke stuff.
        _ = yield self._ensure_namespace_resources()
            ...
            yield exponential_backoff(
                lambda: self.is_pod_running(
                    self.pod_reflector.pods.get(
                        self._get_pod_key(), None)),
                'pod/%s did not start in %s seconds!' % (
                    self.pod_name, self.start_timeout),
                timeout=self.start_timeout,
            )
        except TimeoutError:
            if self._get_pod_key() not in self.pod_reflector.pods:
        ...
        pod = self.pod_reflector.pods[self._get_pod_key()]
    ...
    def stop(self, now=False):
        ...
        try:
            yield exponential_backoff(
                lambda: self.pod_reflector.pods.get(self._get_pod_key(),
                                                    None) is
                None,
    ...
    def _ensure_namespaced_resource_quota(self, quotaspec):
        ...
        try:
            ...
            if e.status != 409:
                self.log.exception("Create resourcequota '%r'" % quota +
                                   "in namespace '%s' " % namespace +
                                   "failed: %s", str(e))
                raise
            else:
