...
class KubeSpawner(Spawner):
    ...
    def _get_pod_url(self, pod):
        ...
        if self.pod_connect_ip:
            hostname = ".".join(
                [
                    s.rstrip("-")
                    for s in self._expand_user_properties(
                        self.pod_connect_ip
                    ).split(".")
                ]
            )

        return "{}://{}:{}".format(proto, hostname, self.port,)
    ...
    def pod_has_uid(self, pod):
        ...
        return bool(
            ...
            and pod["metadata"].get("uid") is not None
        )
    ...
    async def _make_create_resource_request(self, kind, manifest):
        ...
        try:
            ...
            await gen.with_timeout(
                timedelta(seconds=self.k8s_api_request_timeout),
                self.asynchronize(create, self.namespace, manifest,),
            )
        except gen.TimeoutError:
            # Just try again
            return False
    ...
    async def _make_delete_pod_request(
        ...
        try:
            ...
            if e.status == 404:
                self.log.warning(
                    "No pod %s to delete. Assuming already deleted.", ref_key,
                )
    ...
    async def _ensure_namespace(self):
        ...
        try:
            ...
            if e.status != 409:
                # It's fine if it already exists
                self.log.exception(
                    "Failed to create namespace %s", self.namespace
                )
                raise
